export const useIntegrantes = () => {
    const integrantes = [
        {
            subtitulo: "AGOSTO 2025 - JUNIO 2026",
            integrantes: [
                {
                    nombre: "Nicole Arámbula",
                    puesto: "Presidenta",
                    image: "/images/integrantes/25-26/nicole.png",
                    row: 1,
                    position: "left",
                },
                {
                    nombre: "Mariel García",
                    puesto: "Vicepresidenta",
                    image: "/images/integrantes/25-26/mariel.png",
                    row: 1,
                    position: "right",
                },

                {
                    nombre: "Karen Estrada",
                    puesto: "Directora de Marketing",
                    image: "/images/integrantes/25-26/karen.png",
                    row: 2,
                    position: "left",
                },
                {
                    nombre: "Andrea de la Peña",
                    puesto: "Directora de Finanzas",
                    image: "/images/integrantes/25-26/andrea.png",
                    row: 2,
                    position: "center",
                },
                {
                    nombre: "Carolina Cruz",
                    puesto: "Directora de Proyectos",
                    image: "/images/integrantes/25-26/carolina.png",
                    row: 2,
                    position: "right",
                },

                {
                    nombre: "María Fernanda Cuevas",
                    puesto: "Directora de Responsabilidad Social",
                    image: "/images/integrantes/25-26/mafer.png",
                    row: 3,
                    position: "left",
                },
                {
                    nombre: "Azereth Gutiérrez",
                    puesto: "Directora de Vinculación",
                    image: "/images/integrantes/25-26/azereth.png",
                    row: 3,
                    position: "center",
                },
                {
                    nombre: "Fátima Castillo",
                    puesto: "Directora de Software",
                    image: "/images/integrantes/25-26/fatima.png",
                    row: 3,
                    position: "right",
                },
            ],
        },

        {
            subtitulo: "AGOSTO 2024 - JUNIO 2025",
            integrantes: [
                {
                    nombre: "Darianna Téllez",
                    puesto: "Presidenta",
                    image: "/images/integrantes/24-25/dari.png",
                    row: 1,
                    position: "left",
                    linkedin: "https://www.linkedin.com/in/darianna-t%C3%A9llez-777849257/"
                },
                {
                    nombre: "Brisma Alvarez",
                    puesto: "Vicepresidenta",
                    image: "/images/integrantes/24-25/brisma.png",
                    row: 1,
                    position: "right",
                    linkedin: "https://www.linkedin.com/in/brisma-alvarez-valdez/"
                },
                {
                    nombre: "Angela Pesina",
                    puesto: "Directora de Responsabilidad Social",
                    image: "/images/integrantes/24-25/angela.png",
                    row: 2,
                    position: "left",
                    linkedin: "https://www.linkedin.com/in/angelapesina/"
                },
                {
                    nombre: "Jimena Carmona",
                    puesto: "Directora de Marketing",
                    image: "/images/integrantes/24-25/jime.png",
                    row: 2,
                    position: "center",
                    linkedin: "https://www.linkedin.com/in/jimenacarmona/"
                },
                {
                    nombre: "Eunice Peña",
                    puesto: "Directora de Finanzas",
                    image: "/images/integrantes/24-25/eunice.png",
                    row: 2,
                    position: "right",
                },
            ],
        },

        {
            subtitulo: "AGOSTO 2023 - JUNIO 2024",
            integrantes: [
                {
                    nombre: "Sofía García",
                    puesto: "Presidenta",
                    image: "/images/integrantes/23-24/Sofi.png",
                    row: 1,
                    position: "left",
                    linkedin: "https://www.linkedin.com/in/sof%C3%ADa-gigdem-garcia-orta-358378236/"
                },
                {
                    nombre: "Alejandra Coeto",
                    puesto: "Vicepresidenta",
                    image: "/images/integrantes/23-24/Ale.png",
                    row: 1,
                    position: "right",
                    linkedin: "https://www.linkedin.com/in/alecoeto/"
                },
                {
                    nombre: "Angela Pesina",
                    puesto: "Directora de Responsabilidad Social",
                    image: "/images/integrantes/23-24/Angela.png",
                    row: 2,
                    position: "left",
                    linkedin: "https://www.linkedin.com/in/angelapesina/"
                },
                {
                    nombre: "Jimena Carmona",
                    puesto: "Directora de Marketing",
                    image: "/images/integrantes/23-24/Jime.png",
                    row: 2,
                    position: "center",
                    linkedin: "https://www.linkedin.com/in/jimenacarmona/"
                },
                {
                    nombre: "Fernanda Quevedo",
                    puesto: "Directora de Proyectos",
                    fecha: "(febrero-junio 2024)",
                    image: "/images/integrantes/23-24/Fer.png",
                    row: 2,
                    position: "center",
                    linkedin: "https://www.linkedin.com/in/fernanda-quevedo-593415284/"
                },
                {
                    nombre: "Darianna Téllez",
                    puesto: "Directora de Finanzas",
                    fecha: "(febrero-junio 2024)",
                    image: "/images/integrantes/23-24/Dari.png",
                    row: 2,
                    position: "right",
                    linkedin: "https://www.linkedin.com/in/darianna-t%C3%A9llez-777849257/"
                },
                {
                    nombre: "Marcela Jiménez",
                    puesto: "Directora de Finanzas",
                    fecha: "(agosto-diciembre 2023)",
                    image: "/images/integrantes/23-24/Marce.png",
                    row: 3,
                    position: "left",
                    linkedin: "https://www.linkedin.com/in/marcela-jimenez-penarrieta/"
                },
                {
                    nombre: "Ariadna Noli",
                    puesto: "Directora de Proyectos",
                    fecha: "(agosto-diciembre 2023)",
                    image: "/images/integrantes/23-24/Ari.png",
                    row: 3,
                    position: "right",
                },
            ],
        },
    ];

    return integrantes;
};
